...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (useSupabase()) {
      ...
      const mayWithdraw = !locked || type === 'emergency' || !unlockDate || unlockDate <= new Date();
      if (!mayWithdraw) return res.status(400).json({ error: 'Goal is locked until unlock date' });
      if (Number(goal.balance) < amountNumber) return res.status(400).json({ error: 'Insufficient goal balance' });
      await query(`UPDATE goals SET balance = balance - $1 WHERE id = $2`, [amountNumber, goalId]);
      await query(
        `INSERT INTO withdrawal_requests (id, farmer_id, goal_id, amount, currency, status, type, memo, requested_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [id, farmerId, goalId, amountNumber, currency, 'approved', type, memo || null, requestedAt]
      );
      return res.status(201).json({ withdrawal: { id, farmerId, goalId, amount: amountNumber, currency, status: 'approved', type, memo, requestedAt } });
    }

    const db = readLocalDb();
    const farmer = db.farmers.find((f: any) => f.id === farmerId);
    if (!farmer) return res.status(404).json({ error: 'Farmer not found' });
    const goal = db.goals.find((g: any) => g.id === goalId && g.farmerId === farmerId);
    if (!goal) return res.status(404).json({ error: 'Goal not found' });
    const amountNumber = Number(amount);
    const unlockDate = goal.unlockDate ? new Date(goal.unlockDate) : null;
    const mayWithdraw = !goal.locked || type === 'emergency' || !unlockDate || unlockDate <= new Date();
    if (!mayWithdraw) return res.status(400).json({ error: 'Goal is locked until unlock date' });
