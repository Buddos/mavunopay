          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Important updates for your farm savings, loans, and cooperative activity.
              </p>
            </div>
            <button
      ...
      <section className="rounded-xl border bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Track progress, targets, and balances for each goal.
            </p>
          </div>
        ...
        <div className="mt-5 space-y-4">
          {goals.length === 0 ? (
            ...
            goals.map((goal) => {
              ...
              const unlockDate = goal.unlockDate
                ? new Date(goal.unlockDate).toLocaleDateString()
                : null;
              return (
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <div className="text-sm text-slate-500">
                        Target {formatCurrency(target)} · Balance {formatCurrency(balance)}
                      </div>
                      {goal.description && (
                        <div className="mt-2 text-sm text-slate-600">{goal.description}</div>
                      )}
                      {goal.targetDate && (
                      ...
                      {goal.locked && (
                        <div className="mt-1 text-sm text-amber-700">
                          Locked until {unlockDate || "maturity"}
                        </div>
                      )}
                    </div>
        ...
        <div className="rounded-xl border bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                All incoming payments allocated across your goals.
              </p>
            </div>
            <button
              className="rounded-md border border-slate-200 bg-slate-100 px-3 py-2 text-sm"
                ...
                <div key={t.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <div className="text-sm font-semibold text-slate-900">
                        {t.assetCode
                          ? `${t.assetCode} ${Number(t.amount).toLocaleString()}`
                    ...
                    <div className="text-sm text-slate-500">
                      {t.groupPayment ? "Co-op collection" : t.memo || "Automatic allocation"}
                    </div>
                  </div>
                  <div className="mt-3 text-sm text-slate-600">
                    ...
                    {(t.allocations || [])
                      .map(
                        (a: any) =>
                          `${a.key}: ${formatCurrency(Number(a.amount) || 0)} (${a.pct}%)`,
                      )
                      .join(", ")}
                  </div>
                </div>
          ...
          <div className="mt-6 border-t border-slate-200 pt-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Post a payment and automatically allocate funds to your goals.
            </p>
            <SimulatePayment
    ...
    <div className="rounded-xl border bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Your farm-based credit score and borrowing power.
          </p>
        </div>
        <a
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <div className="mt-2 text-2xl font-semibold text-slate-900">
            KES {Number(profile.totalSaved).toLocaleString()}
          </div>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
    ...
    <div className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      {coop ? (
        <div className="mt-4 space-y-3 rounded-2xl bg-slate-50 p-4">
          ...
          <div className="text-sm text-slate-500">
            Savings pool: KES {Number(coop.savingsBalance || 0).toLocaleString()}
          </div>
        ...
        <div className="mt-4 space-y-3 rounded-2xl bg-slate-50 p-4">
          <div className="text-sm text-slate-600">
            No cooperative connected yet. Create one or join with a code.
          </div>
      ...
      {status && (
        <div className="mt-4 rounded-lg bg-slate-100 p-3 text-sm text-slate-700">{status}</div>
      )}
    </div>
  );
}
...
function WithdrawalWidget({
  ...
  return (
    <div className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        <label className="sr-only" htmlFor="withdrawal-goal">
          Select goal
        </label>
        <select
          id="withdrawal-goal"
        ...
        >
          ...
          {goals.map((goal) => (
            <option key={goal.id} value={goal.id}>
              {goal.name} — KES {Number(goal.balance || 0).toLocaleString()}
            </option>
          ))}
        </select>
        <label className="sr-only" htmlFor="withdrawal-amount">
          Withdrawal amount
        </label>
        <input
...
function LoanWidget({ farmerId, onRequested }: { farmerId: string; onRequested: () => void }) {
  ...
  return (
    <div className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">
        Use your credit profile to access working capital.
      </p>
      <div className="mt-4 space-y-3">
        <input
          type="number"
...
function RegisterWidget({ onRegistered }: { onRegistered: (f: any) => void }) {
  ...
  return (
    <div className="mt-4 max-w-md">
      ...
      />
      <label htmlFor="register-name" className="block text-sm font-medium mt-3">
        Name (optional)
      </label>
      <input
        id="register-name"
    ...
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
      ...
      <div className="mt-4 space-y-3 text-sm text-slate-700">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          {locked && (
            <label className="sr-only" htmlFor="unlock-date">
              Unlock date
            </label>
          )}
          {locked && (
