      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Track payments and automatic goal allocations over time.
          </p>
        </div>
        <button
        ...
        <div className="mt-6 space-y-4">
          ...
          {!loading && !error && transactions.length === 0 && (
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-600">
              No transactions yet. Use the dashboard payment simulator or send a live payment to see
              allocations.
            </div>
          )}
            ...
            <div key={tx.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <div className="text-base font-semibold text-slate-900">
                    {formatAmount(Number(tx.amount) || 0, tx.assetCode)}
                  </div>
                ...
                <div className="text-sm text-slate-500">
                  {tx.groupPayment ? "Co-op collection" : tx.memo || "Automatic allocation"}
                </div>
              </div>
              <div className="mt-4 text-sm text-slate-700">
                ...
                {(tx.allocations || [])
                  .map(
                    (alloc: any) =>
                      `${alloc.key}: ${formatAmount(Number(alloc.amount) || 0, alloc.assetCode || tx.assetCode || "KES")} (${alloc.pct}%)`,
                  )
                  .join(", ")}
              </div>
            </div>
